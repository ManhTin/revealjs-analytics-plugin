  ...
  $.slickQuiz = function (element, options) {
    var plugin = this,
      ...
      defaults = {
        ...
        nextSlideText: "Continue Presentation",
        ...
        events: {
          onStartQuiz: (options) => {},
          onCompleteQuiz: (options) => {}, // reserved: options.questionCount, options.score
        },
      },
    ...
    var questions = plugin.config.randomSortQuestions
      ? quizValues.questions.sort(() => Math.round(Math.random()) - 0.5)
    ...
    var internal = {
      method: {
        ...
        turnKeyAndGo: (key, go) => {
          ...
          $.when.apply(null, key).then(() => {
            go();
          });
        },

        // get one jQ
        getKeyNotch: (key, notch) => {
          // notch >= 1, key = []
    ...
    plugin.method = {
      ...
      setupQuiz: function (options) {
        ...
        // add retry button (and one for next slide) to results view, if enabled.
        ...
        for (i in questions) {
          if (questions.hasOwnProperty(i)) {
            ...
            var answers = plugin.config.randomSortAnswers
              ? question.a.sort(() => Math.round(Math.random()) - 0.5)
              : question.a;
      ...
      resetQuiz: (startButton, options) => {
        var key, keyNotch, kN;
        key = internal.method.getKey(1); // how many notches == how many jQ animations you will run
        keyNotch = internal.method.getKeyNotch; // a function that returns a jQ animation callback function
        kN = keyNotch; // you specify the notch, you get a callback function for your animation

        $quizResults.fadeOut(300, () => {
          $(_element + " input")
            .prop("checked", false)
            .prop("disabled", false);
      ...
      checkAnswer: (checkButton, options) => {
        var key, keyNotch, kN;
        ...
        answerSelects.each(function () {
          ...
          selectedAnswers.push(
            Number.parseInt(id.replace(/(.*\_question\d{1,}_)/, ""), 10),
          );
        });
      ...
      nextQuestion: (nextButton, options) => {
        var key, keyNotch, kN;
        key = internal.method.getKey(1); // how many notches == how many jQ animations you will run
        keyNotch = internal.method.getKeyNotch; // a function that returns a jQ animation callback function
        kN = keyNotch; // you specify the notch, you get a callback function for your animation
      ...
      backToQuestion: (backButton, options) => {
        var key, keyNotch, kN;
        key = internal.method.getKey(2); // how many notches == how many jQ animations you will run
        keyNotch = internal.method.getKeyNotch; // a function that returns a jQ animation callback function
        kN = keyNotch; // you specify the notch, you get a callback function for your animation
        ...
        if (responses.css("display") === "block") {
          questionLI.find(_responses).fadeOut(300, () => {
            questionLI
              .removeClass(correctClass)
              .removeClass(incorrectClass)
      ...
      compareAnswers: (trueAnswers, selectedAnswers, selectAny) => {
        if (selectAny) {
    ...
    plugin.init = () => {
      ...
      $(_element + " " + _nextSlideBtn).on("click", (e) => {
        e.preventDefault();
        e.stopPropagation();
